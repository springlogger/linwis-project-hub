...
import type { TableRow, TableSummaryItem } from '../dashboard.data'
  ...
  <div class="flex flex-col gap-4 p-6 pb-8">
    <div class="grid grid-cols-4 gap-3">
      <div
        ...
        >
        <div
          class="flex size-9 shrink-0 items-center justify-center rounded-[0.5625rem] border"
        ...
        <div>
          <div class="mb-0.5 text-[0.6875rem] font-medium text-[var(--color-third)]">{{ item.label }}</div>
          <div class="text-[1.375rem] font-bold tracking-[-0.04em] text-[var(--color-text)]">{{ item.value }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2.5">
      ...
      <div class="flex flex-1 items-center gap-1.5">
        <button
          ...
          :key="filter"
          type="button"
          class="cursor-pointer rounded-[0.4375rem] px-3 py-1.5 text-xs transition-all duration-150"
      ...
      <button type="button" class="app-toolbar-button bg-[var(--color-bg-third)] text-[var(--color-third)] shrink-0 rounded-[0.4375rem]">
        Sort by: Due date
    ...
    <div class="app-panel overflow-hidden">
      <div class="grid grid-cols-[2fr_5.625rem_8.75rem_6.875rem_5.625rem_9.375rem_5.625rem_6.25rem_2.5rem] items-center border-b border-[var(--color-border)] bg-white/[0.02] px-5 py-2.5">
        <div
          v-for="column in tableColumns"
          :key="column"
          class="pr-2 text-[0.625rem] font-bold uppercase tracking-[0.07em] text-[var(--color-third)]"
        >
          {{ column }}
        </div>
      </div>
      ...
      <div
        ...
        class="grid grid-cols-[2fr_5.625rem_8.75rem_6.875rem_5.625rem_9.375rem_5.625rem_6.25rem_2.5rem] cursor-default items-center px-5 py-[0.8125rem] transition-colors duration-150 hover:bg-white/[0.025]"
      ...
      >
            <div class="flex min-w-0 items-center gap-2">
              ...
              <span class="overflow-hidden text-ellipsis whitespace-nowrap text-[0.8125rem] font-medium text-[var(--color-text)]">{{ row.name }}</span>
            </div>

            <div>
              <span
                class="inline-flex items-center whitespace-nowrap rounded px-2 py-[0.1875rem] text-[0.6875rem] font-semibold"
            ...
            <div class="flex items-center gap-[0.4375rem]">
              <div
                class="flex size-5 shrink-0 items-center justify-center rounded-full text-[0.46875rem] font-semibold text-white"
            ...
            <div class="flex items-center gap-2">
              <div class="h-[0.3125rem] flex-1 overflow-hidden rounded-full bg-[var(--color-bg-secondary)]">
                <div class="h-full rounded-full" :style="{ width: `${row.progress}%`, background: getProgressColor(row.progress) }" />
              </div>
              <span class="w-7 shrink-0 text-right text-[0.6875rem] font-semibold text-[var(--color-text-light)]">{{ row.progress }}%</span>
            </div>
            ...
            <div class="text-[0.6875rem] text-[var(--color-third)]">{{ row.updated }}</div>

            <div class="flex justify-center">
              <button type="button" class="flex size-[1.625rem] cursor-pointer items-center justify-center rounded-md text-[var(--color-third)] hover:bg-[var(--color-bg-secondary)] hover:text-[var(--color-text-light)]">
