    ...
    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2 2xl:grid-cols-4">
      <div v-for="column in columns" :key="column.key">
        <div class="mb-2.5 flex items-center gap-1.5">
          ...
          <span class="text-xs font-semibold text-[var(--color-text)]">{{ column.title }}</span>
          <span class="rounded-full bg-[var(--color-bg-secondary)] px-1.5 py-px text-[0.6875rem] text-[var(--color-third)]">{{ column.count }}</span>
        </div>
        ...
        <div class="flex flex-col gap-1.5">
          <div
            ...
            @click="navigateTo(`/app/tasks?task=${task.id}`)"
          >
            <div class="mb-1.5">
              <div class="mb-1 text-xs font-medium leading-snug text-[var(--color-text)]">{{ task.title }}</div>
              <span
            ...
            <div class="flex items-center justify-between gap-2">
              <div class="flex min-w-0 items-center gap-1">
                <div
                  class="flex h-[1.125rem] w-[1.125rem] shrink-0 items-center justify-center rounded-full text-[0.4375rem] font-semibold text-white"
                  :style="{ background: getAvatarBackground(task.assignee) }"
                >
                  {{ getAvatarInitials(task.assignee) }}
                </div>
                <span class="truncate text-[0.625rem] text-[var(--color-third)]">{{ task.assignee.split(' ')[0] }}</span>
              </div>
              <span class="shrink-0 text-[0.625rem] text-[var(--color-third)]">{{ task.due }}</span>
            </div>
          </div>
        </div>
      </div>
